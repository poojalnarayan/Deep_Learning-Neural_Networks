...
def test_toy_rnn(capsys):
    ...
    with capsys.disabled():
        print("{:.1f} RMSE for RNN on toy problem".format(rmse))
    assert rmse < 2
...
def test_image_cnn(capsys):
    ...
    with capsys.disabled():
        print("{:.1%} accuracy for CNN on MNIST sample".format(accuracy))
    assert accuracy > 0.8
...
def test_text_rnn(capsys):
    ...
    with capsys.disabled():
        print("{:.1%} accuracy for RNN on Youtube comments".format(accuracy))
    assert accuracy > 0.8
...
def test_text_cnn(capsys):
    ...
    accuracy = binary_accuracy(model.predict(test_in), test_out)
    with capsys.disabled():
        print("{:.1%} accuracy for CNN on Youtube comments".format(accuracy))
    assert accuracy > 0.8
