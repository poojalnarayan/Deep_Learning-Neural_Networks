...
def test_predict():
    ...
    [[h11, h12],
     [h21, h22]] = [[s(13 * .1 + -14 * -.3 + 15 * .5),
                    ...
                    [s(-16 * .1 + 17 * -.3 - 18 * .5),
                     s(-16 * -.2 + 17 * .4 + -18 * -.6)]]
    npt.assert_allclose(predictions, np.array(
        [[s(h11 * -.7 + h12 * .10),
          s(h11 * .8 + h12 * -.11),
          s(h11 * -.9 + h12 * .12)],
         [s(h21 * -.7 + h22 * .10),
...
def test_gradients():
    ...
    oi = [[h11 * .2 + h12 * .4 + h13 * .6, h11 * -.2 + h12 * -.4 + h13 * -.6],
    ...
    npt.assert_allclose(hidden_to_output_gradient, np.array(
    ...
    npt.assert_allclose(input_to_hidden_gradient, np.array(
...
def test_train_greater_than_half():
    inputs = nprand.uniform(size=(100, 1))
